...
def gatherStatistics(allPolygons, allEdges):
    ...
    if stats['orange'] > 0:
        stats['string'] = stats['size'] + '_y' + str(stats['grey']) + 'b' + str(stats['blue']) + 'g' + str(stats['green']) + 'o' + str(stats['orange']) + '_k' + str(stats['black']) + 'r' + str(stats['red'])
    else:
...
def switchColors(allPolygons, colorSet):
    if 'orange' in colorSet:
        ...
        for p in allPolygons:
            if allPolygons[p].color == invertDict[3].split('_')[1]:
...
def computeDisconnectivityScoreH(po : ParamsObjective):
    ...
    if 'orange' in po.colorSet:
        mySum = mySum + (1.1 * math.fabs(stats['grey'] - po.objectiveColors['grey']) + 0.98 * math.fabs(stats['blue'] - po.objectiveColors['blue']) + 0.95 * math.fabs(stats['green'] - po.objectiveColors['green']) + 0.92 * math.fabs(stats['orange'] - po.objectiveColors['orange'])) * 6
    else:
...
class OneDatabaseItem:
    def __init__(self, theDate, hash_id, template, rle, b64, p, e, y, b, g, o, k, r, detailedStats):
        ...
        self.k = k
        self.r = r
        self.filename = self.template.replace('_', '') + '_' + self.hash_id + '_y' + str(self.y) + '_b' + str(self.b) + '_g' + str(self.g) + '_o' + str(self.o) + '_k' + str(self.k) + '_r' + str(self.r)
...
def addEntryToDatabase(filepathParquet, allPolygons, allEdges, templateName):
    ...
    odi = OneDatabaseItem(theDate, hash_id, templateName, rle, b64, len(allPolygons), len(allEdges), stats['grey'], stats['blue'], stats['green'], stats['orange'], stats['black'], stats['red'], str(detailedStats))
    ...
    for hash_id in hashesDict:
        ...
        if 'orange' in hashesDict[hash_id]:
            ...
            # if min(hashesDict[hash_id]['grey'].keys()) == 1 or min(hashesDict[hash_id]['blue'].keys()) == 1 or min(hashesDict[hash_id]['green'].keys()) == 1 or min(hashesDict[hash_id]['orange'].keys()) == 1:
                # database = database.filter(pl.col('hash_id') != hash_id)
                # print(hash_id, 'orange' in hashesDict[hash_id])
            if max(hashesDict[hash_id]['grey'].keys()) > 4 or max(hashesDict[hash_id]['blue'].keys()) > 4 or max(hashesDict[hash_id]['green'].keys()) > 4 or max(hashesDict[hash_id]['orange'].keys()) > 4:
                database = database.filter(pl.col('hash_id') != hash_id)
                print(hash_id, 'orange' in hashesDict[hash_id])
