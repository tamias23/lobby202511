...
async function initReplay() {
    try {
        ...
        if (gamesList.length > 0) {
            ...
            selector.innerHTML = "<option>No Games in Parquet</option>";
            document.querySelector("h1").innerText += " (No Games in Parquet)";
        }
    } catch (err) {
        console.error(err);
    }
...
async function fetchTurn(turn) {
    if (turn < 0) turn = 0;
    if (turn > totalTurns) turn = totalTurns;
    currentTurn = turn;
...
function drawHUD() {
    let ui = document.getElementById("replay-hud");
    if (!ui.hasChildNodes()) {
        const btnPrev = document.createElement("button");
        btnPrev.innerHTML = "◀ Prev";
        btnPrev.style = "padding: 8px 16px; font-size: 15px; cursor: pointer; background: #374151; color: white; border: 1px solid #4b5563; border-radius: 6px; font-weight: bold;";
        btnPrev.onclick = () => fetchTurn(currentTurn - 1);
        
        const btnNext = document.createElement("button");
        btnNext.innerHTML = "Next ▶";
        btnNext.style = "padding: 8px 16px; font-size: 15px; cursor: pointer; background: #374151; color: white; border: 1px solid #4b5563; border-radius: 6px; font-weight: bold;";
        btnNext.onclick = () => fetchTurn(currentTurn + 1);
...
function renderBoard(board) {
    ...
    if (!boardLayer) {
        boardLayer = document.createElementNS("http://www.w3.org/2000/svg", "g");
        boardLayer.id = "board-layer";
        svgCanvas.appendChild(boardLayer);
        ...
        for (const polyKey in board.allPolygons) {
            ...
            boardLayer.appendChild(polygonEl);
        }
    ...
    for (const pieceKey in board.allPieces) {
        ...
        let g = document.getElementById(`piece-${pieceKey}`);
        let scale = 1.0;
        ...
        if (!g) {
            ...
            let pLayer = document.getElementById("pieces-layer");
