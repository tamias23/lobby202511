...
describe('7-Turn Game Simulation with Rotations', () => {
  beforeEach(() => {
    const mockBoard = {
      ...
      allPieces: {
        'white_heroe_0': { position: 'poly_1', side: 'white', type: 'heroe', canMove: 1, centerTransform: [0, 0] }
      }
    };
    setBoard(mockBoard);
  ...
  test('Simulation of 7 full turns (14 half-turns) with rotations', () => {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      if (boardstate.turn === 5 && boardstate.whoseTurnItIs === 'black' && boardstate.boardRotated === 'yes') {
        rotate();
      }
    ...
    expect(sequence[1]).toEqual({ turn: 1, halfTurn: 2, player: 'black', rotated: 'no' });
    ...
    expect(sequence[3].rotated).toBe('yes'); // Turn 2 Black
    expect(sequence[8].rotated).toBe('yes'); // Turn 5 White
    
    // Turn 5 Black: rotate() is called back to 'no'
